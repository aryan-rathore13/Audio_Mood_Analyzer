...
const FileUploadButton = ({ onFileChange }: { onFileChange: (file: File | null) => void }) => {
  ...
  return (
    <div className="relative">
      ...
      {fileName && (
        <p className="mt-2 text-sm font-medium text-[#1ed760] dark:text-[#1ed760] truncate max-w-full px-1">
          Uploaded: {fileName}
        </p>
      )}
    </div>
...
const Dashboard = () => {
  ...
  const getSuggestions = async () => {
    ...
    setIsLoading(true);
    console.log("Sending to /suggest:", new URLSearchParams({
      prompt: prompt,
      sessionId: sessionId,
    }).toString());
  ...
  const analyzeAudio = async () => {
    ...
    try {
      const response = await fetch(`${import.meta.env.VITE_API_URL}/analyze-audio`, {
        method: "POST",
          ...
          <div className="bg-white dark:bg-gray-800 rounded-lg shadow-md p-6">
            ...
            <div className="mt-4 relative inline-block">
              <motion.button
                ...
                disabled={isLoading}
                className="relative px-6 py-2 bg-[#1db954] text-white rounded-md hover:bg-[#1ed760] transition-colors disabled:opacity-50 z-10"
              >
                {isLoading ? (
                  <svg className="animate-spin h-5 w-5 mx-auto text-white" viewBox="0 0 24 24">
                    <circle className="opacity-25" cx="12" cy="12" r="10" stroke="currentColor" strokeWidth="4" fill="none" />
              ...
              {audioFile && !isLoading && (
                <MovingBorder duration={3000} rx="8px" ry="8px" className="h-12 w-12 opacity-80 bg-[radial-gradient(#1ed760_40%,transparent_60%)]" />
              )}
            </div>
            {audioResult && (
